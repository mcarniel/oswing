 ...
 * it creates an editable grid frame with additional column headers.</p>
...
public class ClientApplication {
  ...
  public ClientApplication() {
    ...
    props.setProperty("...","...");
    props.setProperty("Number Values","Number Values");
    props.setProperty("Text Values","Text Values");
    props.setProperty("Lookup","Lookup");
  ...
  private void createConnection() {
    try {
      ...
      try {
        stmt = conn.prepareStatement("create table DEMO20(TEXT VARCHAR,FORMATTED_TEXT VARCHAR,DECNUM DECIMAL(10,2),CURRNUM DECIMAL(10,2),THEDATE DATE,COMBO VARCHAR,CHECK_BOX CHAR(1),RADIO CHAR(1),CODE VARCHAR,INT_VALUE NUMERIC(6),PRIMARY KEY(TEXT))");
        stmt.execute();
        stmt.close();

        stmt = conn.prepareStatement("create table DEMO20_LOOKUP(CODE VARCHAR,DESCRCODE VARCHAR,PRIMARY KEY(CODE))");
        stmt.execute();
        ...
        for(int i=0;i<900;i++) {
          stmt.close();
          stmt = conn.prepareStatement("insert into DEMO20_LOOKUP values('A"+i+"','ABCDEF"+String.valueOf((char)(i+78))+"')");
          stmt.execute();
        }
