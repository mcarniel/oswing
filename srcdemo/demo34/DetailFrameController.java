...
public class DetailFrameController extends FormController {
  ...
  public Response loadData(Class valueObjectClass) {
    ...
    try {
      ...
      ResultSet rset = stmt.executeQuery("select DEMO34.TEXT,DEMO34.DECNUM,DEMO34.CURRNUM,DEMO34.THEDATE,DEMO34.COMBO,DEMO34.CHECK_BOX,DEMO34.RADIO,DEMO34.CODE,DEMO34_LOOKUP.DESCRCODE,DEMO34.TA from DEMO34,DEMO34_LOOKUP where TEXT='"+pk+"' and DEMO34.CODE=DEMO34_LOOKUP.CODE");
      if (rset.next()) {
  ...
  public Response insertRecord(ValueObject newPersistentObject) throws Exception {
    ...
    try {
      stmt = conn.prepareStatement("insert into DEMO34(TEXT,DECNUM,CURRNUM,THEDATE,COMBO,CHECK_BOX,RADIO,CODE,TA) values(?,?,?,?,?,?,?,?,?)");
  ...
  public Response updateRecord(ValueObject oldPersistentObject,ValueObject persistentObject) throws Exception {
    PreparedStatement stmt = null;
    try {
      stmt = conn.prepareStatement("update DEMO34 set TEXT=?,DECNUM=?,CURRNUM=?,THEDATE=?,COMBO=?,CHECK_BOX=?,RADIO=?,CODE=?,TA=? where TEXT=?");
