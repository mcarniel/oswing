...
public class EmpDetailController extends FormController {
  ...
  public EmpDetailController(Connection conn) {
    this.conn = conn;
...
//      frame.getMainPanel().setMode(Consts.INSERT);
    ...
    frame.getMainPanel().linkGrid(frame.getParentGrid(),pk,true,true,true,frame.getNavigatorBar());
  ...
  public Response loadData(Class valueObjectClass) {
    try {
      ...
      int row = frame.getParentGrid().getSelectedRow();
      if (row!=-1) {
        GridEmpVO gridVO = (GridEmpVO)frame.getParentGrid().getVOListTableModel().getObjectForRow(row);
...
*/
  ...
  public Response deleteRecord(ValueObject persistentObject) throws Exception {
    ...
    try {
      ...
      stmt.setString(1,vo.getEmpCode());
      stmt.execute();
      frame.getParentGrid().reloadData();
